...
describe("dev server integration", () => {
  ...
  it("cleans up generated files orphaned while the server was down", async () => {
    const root = makeProject();
    const orphan = path.join(root, "src", "i18n", "fr.ts");
    fs.writeFileSync(
      orphan,
      "/* Generated by vite-plugin-i18n-codegen — do not edit manually */\nexport default {};\n",
    );
    const handWritten = path.join(root, "src", "i18n", "helpers.ts");
    fs.writeFileSync(handWritten, "export const noop = () => {};\n");
  ...
  it("reports an error instead of deleting the barrel when the last locale is removed", async () => {
    const root = makeProject();
    const server = await createServer({
      root,
      logLevel: "error",
    ...
    try {
      ...
      fs.rmSync(path.join(root, "src", "i18n", "resources", "en.json"));
      await vi.waitFor(() => {
        expect(
          loggerError.mock.calls.some(([message]) =>
            String(message).includes("No .json translation resources found"),
          ),
        ).toBe(true);
      });
      expect(fs.existsSync(indexFile)).toBe(true);
