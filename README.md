...
```bash
# NPM
npm i -D vite-plugin-i18n-codegen
```
...
export const dictionary: Dictionary = { de_DE, en_US };
```
...
// de_DE.json: "{place} begrüßt {name}!"       -> (name, place)
...
Generated files use `import type` for the `Translations` barrel, so the output type-checks under `verbatimModuleSyntax` and `moduleResolution: "node16" | "nodenext" | "bundler"`.
